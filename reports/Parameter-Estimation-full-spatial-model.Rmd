...
centroids <- here::here("data", "Geography/GravityModel/raw/centroids.tsv") %>%
...
change_at <- seq(from = time_window + 1, to = T, by = time_window)
num_Rjt   <- length( change_at) + 1
rvector   <- rep(seq_len(num_Rjt), each = N) 
rindex    <- makeRmatrix(rvector, nrow = T, ncol = N,
                         change_at = change_at)
...
outname <- paste0(ADM0, "-", t.proj, "-", n_days, ".png")
