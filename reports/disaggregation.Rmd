...
```
Aggregate Western Area Rural and Western Area Urban into Western to
allow for comparison with linelist.

```{r western, eval = method[["restim"]]}

who_wide$WESTERN <- who_wide$`WESTERN AREA RURAL` +
    who_wide$`WESTERN AREA URBAN`

who_wide <- select(who_wide,
                   -`WESTERN AREA URBAN`,
                   -`WESTERN AREA RURAL`)
...
district_weekly$district <- stringr::str_replace_all(projections$district,
                                                 "PORT.LOKO",
                                                 "PORTLOKO")
```
