...
start           <- WHO_Liberia_wide$Date[t.proj] - 7
end             <- max(dates.all) 
...
weekly_incid    %<>% tidyr::gather(key = NAME_1, value = incid, factor_key = TRUE, -c(Date, week, Category))

liberia.df$NAME_1 <- plyr::revalue(liberia.df$NAME_1, c("GBAPOLU"="GBARPOLU"))

incid_map  <- left_join(liberia.df, weekly_incid)
...
p <- ggplot(incid_map) + 
    ...
    coord_equal() +
    facet_grid(week~Category) +
    scale_fill_gradient2(low = "grey50", high = "firebrick")
