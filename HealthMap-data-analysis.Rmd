...
p <- ggplot(by.location[["Liberia"]], aes(Date, Cases)) +
    ...
    geom_point(size = 1.1, stroke = 0, shape = 16, colour = "gray") +
...
p <- p + theme_bw() +
        theme(panel.border = element_blank(),
              ...
              axis.line = element_line(colour = "black"),
              strip.background = element_blank())
    p <- p + theme(axis.text.x = element_text(angle = 80, hjust = 1, size = 5))
    p <- p + theme(legend.title = element_blank())
    p <- p + xlab("")
p <- p + scale_x_date(date_labels =  "%b %Y")
p <- p + theme(strip.text.x = element_text(size = 3, face = "bold"))
p <- p + theme(axis.text.y = element_text(size = 1))
