...
params:
  ...
  risk: !r c("flow_from_mbandaka_1_1_1_1.csv","flow_from_équateur_2.01_1.3_1.12.csv")
...
eq_infectivity <- params$R * EpiEstim::overall_infectivity(incid$eq, si)
mb_infectivity <- params$R * EpiEstim::overall_infectivity(incid$mb, si)
...
```{r, eval = FALSE}
pmovement$eq$expected <- pmovement$eq$pij * c(mean_eq)
pmovement$mb$expected <- pmovement$mb$pij * c(mean_mb)
