...
```{r}
t.proj    <- 175L
start     <- 1:(length(SL_wide$Date) - time_window)
end       <- start + time_window
...
adm1_centroids <- "data/Geography/GravityModel/raw/adm2.txt" %>%
                   read.csv(stringsAsFactors = FALSE,
                            sep = "\t",
                            header = TRUE) %>%
...
adm1_centroids$ADM2 %<>% gsub("Western Urban", "Western", .)
adm1_centroids$ADM2  %<>%
    gsub(' ', '', .) %<>%
    toupper %<>%
    factor

levels(adm1_centroids$ADM2) <- levels(WHO_SL$CL_DistrictRes)
adm1_centroids %<>% arrange(ADM2)
...
flow.matrix  <- flow_matrix(longitude   = adm1_centroids[, "Centroid_Lon"],
                            ...
                            place.names = adm1_centroids[, "ADM2"],
                            model = "gravity", K = K,
