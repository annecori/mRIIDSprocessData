...
start     <- 2:(length(by.location_incid$Date) - time_window)
end       <- start + time_window
end.dates <- by.location_incid[end, "Date"]
r.estim   <- select(by.location_incid, -Date) %>%
...
tmp <- reshape(projections_distr, idvar='Date',
               timevar='Country',direction='wide')
tmp$Projections_from <- by.location_incid[t.proj,]$Date
outfile   <-  paste0("hm-dates_sim-",  n.dates.sim, ".csv")
outfile   <- here::here("output", outfile) 
