...
adm0 <- params$ADM0
from <- params$from
method <- list("pop_density" = FALSE, "gm" = FALSE, "restim" = TRUE)
...
who_wide <- here::here("data/CaseCounts/processed",
    ...
    readr::read_csv(.) %>%
    filter(Date > "2013-12-31" & Date < "2014-05-29")
weeks <- stringr::str_pad(lubridate::week(who_wide$Date),
                          2,
                          pad = "0")

who_wide$week_of_year <- paste0("2014-W", weeks)
```
