...
therefore the `signature' of movement in inceidence data is more identifyable at finer scale).
...
Given this likelihood, we can write the joint posterior distribution of the parameter given the observaed data as:
\[
...
Variants of the model would have distincts number of parameters, for instance assuming $\gamma =1$ would reduce the number
...
it is justifed to estimate a parameter $\gamma$ (and not assume unity) if and only if the fit of our model to the observed 
...
the Deviance Information Criterion (DIC). Such DIC is a well established measure of goodness of fit, and commomly 
used for model selection. it promotes models that can best reproduce observed patterns while penalising
...
\begin{itemize}
\item a simple model selection procedure,
\item  a multi-model averaging procedure.
\end{itemize}
...
While selecting a model is easier to implement, multi-model averaging would better
accounts for both parameter and model uncertainty, i.e. parametric and structural uncertainties.
