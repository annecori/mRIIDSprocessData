...
plots.list         <- lapply(colnames(incidence.count), function(location){
...
```{r}
by_date <- split(weekly.projections, weekly.projections$Date)      
...
tmp <- tidyr::gather(weekly.available, Country, Incidence, -c(Category, Date)) 
    ggplot(tmp, aes(Date, Incidence, color = Category)) +
    geom_point() +
    theme_minimal() + facet_wrap(~Country) +
        geom_line(data = projections_distr, aes(x = Date, y = y, group = 1),
                  color = 'black', size = 1) +    
        geom_ribbon(data = projections_distr, aes(x = Date,
                                                  ymin = ymin,
                                                  ymax = ymax,
                                                  group = 1),
                                               inherit.aes = FALSE,
