...
```{r child = 'WHO-data-analysis-Sierra.Rmd', eval = TRUE}
```
...
```{r child = 'WHO-data-analysis-alldistricts.Rmd', eval = FALSE}
```

## Evaluating goodness of fit
```{r, eval = FALSE}
outfile    <- "output/sierra-lh.csv"
log_likelihood  <- read.csv(outfile)

normalise  <- function(col){
    normx <- norm(col, "2")
    if(normx != 0) col <- col/normx
    col
}
normalised <- select(log_likelihood, -c(pow_dist, p.stay)) %>%
              apply(2, normalise) %>% rowSums

pstay <- unique(log_likelihood$p.stay)   %>% sort
power <- unique(log_likelihood$pow_dist) %>% sort
log_likelihood   <- matrix(normalised, nrow = length(power), byrow = TRUE)
p <- plotly::plot_ly(x = ~pstay, y = ~power, z = ~log_likelihood) %>% plotly::add_surface()
