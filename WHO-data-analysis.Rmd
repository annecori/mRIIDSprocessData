...
analysis.
```{r}
WHO_guinea$CL_DistrictRes %<>% plyr::mapvalues(from = "N'ZEREKORE", to = "NZEREKORE")
...
```{r}
adm1_centroids <- "data/Geography/GravityModel/raw/adm1_centroids.tsv" %>%
                   read.csv(stringsAsFactors = FALSE,
                            sep = "\t",
                            header = TRUE) %>%
                    filter(ADM0 == "Guinea")

flow.matrix           <- flow_matrix(longitude   = adm1_centroids[, "lon"],
                                     latitude    = adm1_centroids[, "lat"],
                                     population  = adm1_centroids[, "pop"],
                                     place.names = adm1_centroids[, "country"],
                                     model = "gravity",
