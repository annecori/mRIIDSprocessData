...
who_wide <- filter(who_wide, week_of_year <= bin_from)
```
...
who_weekly$Date <- as.Date(who_weekly$Date)
mindate <- as.Date(min(sl_large$Date)) - 30
maxdate <- max(sl_large$Date)
comparison <- filter(who_weekly,
                     Date >= mindate & 
                     Date <= maxdate) %>%
...
p <- p + theme_classic()
p
paste0(names(method)[which(method == TRUE)], ".png") %>%
    ggsave(p)
```
